...
// Special Orders 66 has no battery column, so on its own it cannot place a man in
// Battery C. The morning reports can: they are Battery C's own cards. Matching the
// two on serial number resolves the battery for part of this list.
//
// An exact serial match is treated as confirmation. A match on surname where the
// two serials differ by one or two characters is recorded as probable — microfilm
// digits are genuinely ambiguous — and names both readings so a human can adjudicate.
const REPORTS = resolve(ROOT, "data/morning-reports.jsonl");
const cards = readFileSync(REPORTS, "utf8").split("\n").filter((l) => l.trim()).map((l) => JSON.parse(l));

const bySerial = new Map();
const bySurname = new Map();
for (const card of cards) {
  for (const person of card.personnel ?? []) {
    if (!person.name || /^\d+ (EM|Officers)/i.test(person.name) || /^Above/i.test(person.name)) continue;
    const serial = (person.serial ?? "").replace(/^O-?/i, "");
    const surname = person.name.toLowerCase().split(",")[0].replace(/[^a-z]/g, "");
    const entry = { name: person.name, serial, date: card.date, grade: person.grade };
    if (serial) (bySerial.get(serial) ?? bySerial.set(serial, []).get(serial)).push(entry);
    (bySurname.get(surname) ?? bySurname.set(surname, []).get(surname)).push(entry);
  }
}
...
for (const p of people) {
  ...
  const exact = bySerial.get(asn);
  if (exact?.length) {
    p.batteryC = {
      status: "confirmed",
      basis: "serial number appears in the Battery C morning reports",
      firstSeen: exact.map((e) => e.date).sort()[0],
      lastSeen: exact.map((e) => e.date).sort().at(-1),
  ...
  const near = (bySurname.get(surname) ?? []).find((e) => e.serial && charDistance(asn, e.serial) <= 2);
  if (near) {
    p.batteryC = {
      status: "probable",
...
const payload = {
  order: {
    ...
    note:
      ...
      "Where the morning reports name the same serial number, the batteryC field records that " +
      "the man is placed in Battery C by the cross-reference rather than by this order.",
  },
  columns: {
